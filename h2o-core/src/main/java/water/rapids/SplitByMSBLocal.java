  ...
  @Override public void map(Chunk chk[]) {
    ...
    for (int r=0; r<chk[0]._len; r++) {    // tight, branch free and cache efficient (surprisingly)
     ...
     //long thisx = 0;
      ...
      if (!chk[0].isNA(r)) {
        thisx = MathUtils.convertDouble2BigInteger(chk[0].atd(r));
        ...
        if (_isLeft && _id_maps[0]!=null) { // dealing with enum columns
          ...
          thisx = MathUtils.convertDouble2BigInteger(chk[0].atd(r)).subtract(_base[0]).add(BigInteger.ONE);
          MSBvalue = thisx.shiftRight(_shift).intValue();
        }
      }
      ...
      for (int c=1; c<chk.length; c++) {  // TO DO: left align subsequent
        ...
        thisx = MathUtils.convertDouble2BigInteger(chk[c].atd(r));         // TODO : compress with a scale factor such as dates stored as ms since epoch / 3600000L
        if (_isLeft && _id_maps[c] != null) thisx = BigInteger.valueOf(_id_maps[c][(int)chk[c].at8(r)] + 1);
        else {
          thisx =  MathUtils.convertDouble2BigInteger(chk[c].atd(r)).subtract(_base[c]).add(BigInteger.ONE);
        }
